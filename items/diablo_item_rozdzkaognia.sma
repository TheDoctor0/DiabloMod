...
public plugin_init()
{
	...
	diablo_register_item( "Ognista Rozdzka" , 250 );
}
...
public diablo_item_set_data( id ){
	iPromien[ id ]	=	random_num( 50 , 100 );
	skillUsed[ id ]	=	false;
}

public diablo_item_info( id , szMessage[] , iLen , bool:bList ){
	if( bList ){
		formatex( szMessage , iLen , "Mozesz wyczarowac ognista kule uzywajac tego przedmiotu. Zabije ona ludzi w zasiegu . Im wiecej masz inteligencji tym wieksze zadasz obrazenia")
	}
	else{
...
public diablo_item_skill_used( id ){
	...
	if (is_user_alive(id))
	{
		...
		entity_set_string(fEntity,EV_SZ_classname,"fireball4")
...
{	
	...
	if(equal(szClassName, "fireball4"))
	{
