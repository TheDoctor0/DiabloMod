...
#define PLUGIN	"Medium silver bag"
...
public plugin_init()
{
	...
	diablo_register_item( "Sredni Worek Srebra" , 250 );
	
	register_think("Effect_MShield2","Effect_MShield_Think")
}
...
public diablo_item_skill_used( id ){
	if( usedItem[ id ] ){
		...
		if (find_ent_by_owner(-1,"Effect_MShield2",id) > 0)
			return PLUGIN_CONTINUE
		
		new ent = Spawn_Ent("info_target")
		set_pev(ent,pev_classname,"Effect_MShield2")
		set_pev(ent,pev_owner,id)
		set_pev(ent,pev_solid,SOLID_NOT)		
		set_pev(ent,pev_nextthink, halflife_time() + 0.1)
		
		if(!(diablo_is_this_class(id,"Ninja")))
			diablo_set_user_render( id,kRenderFxGlowShell,0,0,0,kRenderNormal,16 , 0.0 )
		else
