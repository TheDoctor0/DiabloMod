...
#define PLUGIN	"Ninja ring"
#define AUTHOR	"O'Zone"
#define VERSION	"1.0"

#define CS_PLAYER_HEIGHT 72.0

new Float:fTime[ 33 ], Float:fTime2 [ 33 ], bool:bHas[ 33 ];

public plugin_init()
{
	register_plugin(PLUGIN, VERSION, AUTHOR)
	
	diablo_register_item( "Opaska Ninjy" );
}

public diablo_item_give( id , szRet[] , iLen ){
	formatex( szRet , iLen , "Twoj noz pozwala ci teleportowac sie co 3 sekundy i zrobic dlugi skok jak kucniesz na 3 sekundy" )
...
public diablo_item_set_data( id ){
	fTime2[ id ]	=	get_gametime() + 3.0;
	
	bHas[ id ]	=	true;
}

public diablo_copy_item( iFrom , iTo ){
	fTime2[ iTo ]	=	get_gametime() + 3.0;
...
public diablo_item_info( id , szMessage[] , iLen , bool:bList ){
	if( bList ){
		formatex( szMessage , iLen , "Twoj noz pozwala ci teleportowac sie co 3 sekundy i zrobic dlugi skok jak kucniesz na 3 sekundy")
	}
	else{
		formatex( szMessage , iLen , "Twoj noz pozwala ci teleportowac sie co 3 sekundy i zrobic dlugi skok jak kucniesz na 3 sekundy" )
	}
...
public diablo_preThinkItem( id ){
	if( is_user_alive( id ) && bHas[ id ] && !diablo_is_freezetime()){
		if( get_gametime() > fTime2[ id ] && get_user_button(id) & IN_ATTACK2 && !(get_user_oldbutton(id) & IN_ATTACK2) && get_user_weapon( id ) == CSW_KNIFE){
			fTime2[ id ] = get_gametime() + 3.0 ;
