...
#define PLUGIN	"Mnich ring"
#define AUTHOR	"O'Zone"
#define VERSION	"1.0"

new iHeal[ 33 ] , bool:bUsedItem[ 33 ] , iGrenade[ 33 ] , sprite_white;

public plugin_init()
{
	register_plugin(PLUGIN, VERSION, AUTHOR)
	
	diablo_register_item( "Zwoj Mnicha" , 250 );
...
public diablo_item_give( id , szRet[] , iLen ){
	formatex( szRet , iLen , "Masz 1/%i szansy na natychmiastowe zabicie z HE. Twoje hp bedzie sie regenerowac co 5 sekund oraz mozesz polozyc leczacy totem na 7 sekund" , iGrenade[ id ] )
}
...
public diablo_item_set_data( id ){
	iHeal[ id ]			=	random_num( 20 , 35 );
	iGrenade[ id ]	=	random_num( 1 , 4 );
	bUsedItem[ id ]		=	false;
}
...
public diablo_item_info( id , szMessage[] , iLen , bool:bList ){
	if( bList ){
		formatex( szMessage , iLen , "Masz szanse na natychmiastowe zabicie z HE. Twoje hp bedzie sie regenerowac co 5 sekund oraz mozesz polozyc leczacy totem na 7 sekund")
	}
	else{
		formatex( szMessage , iLen , "Masz 1/%i szansy na natychmiastowe zabicie z HE. Twoje hp bedzie sie regenerowac co 5 sekund oraz mozesz polozyc leczacy totem na 7 sekund" , iGrenade[ id ] )
	}
}

public diablo_upgrade_item( id ){
	iHeal[ id ] += random_num( 0 , 3 );
