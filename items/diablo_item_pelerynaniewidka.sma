...
#define PLUGIN	"Invisibility Rope"
...
public plugin_init()
{
	...
	diablo_register_item( "Naszyjnik Niewidzialnosci" , 250 );
}

public plugin_natives()
{
	register_native("diablo_get_user_render_rope","getUserRender");
}
...
public diablo_item_set_data( id ){
	iNiewidka[ id ]	=	random_num(150,200);
	bHas[ id ] = true;
	new szClass[ 64 ];
	diablo_get_class_name( diablo_get_user_class( id ) , szClass , charsmax( szClass ) ) ;
	if( !equal( szClass , "Ninja" ) )
