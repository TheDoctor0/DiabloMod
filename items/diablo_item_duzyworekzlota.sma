...
public plugin_init()
{
	...
	diablo_register_item( "Duzy Worek Zlota" , 250 );
	
	register_think("Effect_MShield3","Effect_MShield_Think")
}
...
public diablo_item_set_data( id ){
	iMoney[ id ]	=	random_num( 1200 , 3000 );
	usedItem[ id ]	=	false;
}
...
public diablo_item_skill_used( id ){
	if( usedItem[ id ] ){
		...
		if (find_ent_by_owner(-1,"Effect_MShield3",id) > 0)
			return PLUGIN_CONTINUE
		
		new ent = Spawn_Ent("info_target")
		set_pev(ent,pev_classname,"Effect_MShield3")
