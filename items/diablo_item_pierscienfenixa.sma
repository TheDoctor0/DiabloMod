...
public plugin_init()
{
	...
	diablo_register_item( "Pioro Feniksa" , 250 );
}
...
public diablo_item_set_data( id ){
	iRespawn[ id ]	=	random_num( 3 , 6 );
}

public diablo_item_info( id , szMessage[] , iLen , bool:bList ){
	if( bList ){
		formatex( szMessage , iLen , "Masz 1/x szanse do odrodzenia sie po zgonie")
	}
	else{
		formatex( szMessage , iLen , "Masz 1/%i szanse do odrodzenia sie po zgonie" , iRespawn[ id ] )
	}
}
