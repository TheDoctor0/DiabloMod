...
public plugin_init()
{
	...
	diablo_register_item( "Totem Zdrowia" , 250 );
	
	register_think("Effect_Teamshield2","Effect_Teamshield_Think")
}
...
public diablo_item_skill_used( id ){
	...
	if ( bUsed[ id ] ){
		...
		if (find_ent_by_owner(-1,"Effect_Teamshield2",id) > 0)
			return PLUGIN_CONTINUE
	
		diablo_add_hp( target , iHp[ id ] );
		
		new ent = Spawn_Ent("info_target")
		set_pev(ent,pev_classname,"Effect_Teamshield2")
...
public diablo_damage_taken_pre( iKiller , iVictim , &Float:fDamage ){
	new owner = find_owner_by_euser(iVictim,"Effect_Teamshield2")
