...
#define PLUGIN	"Darksteel Gaunlet"
...
public plugin_init()
{
	...
	diablo_register_item( "Rekawica Zabojcy" , 250 );
	
	RegisterHam(Ham_TakeDamage, "player", "TakeDamage");
}

public diablo_item_give( id , szRet[] , iLen ){
	formatex( szRet , iLen , "Dodatkowe %i obrazen, gdy trafisz kogos od tylu", floatround(15+diablo_get_user_str(id)*2*iDamage[id]/10.0))
}

public diablo_item_set_data( id ){
	iDamage[ id ]	=	random_num( 5 , 9 );
}
...
public diablo_item_info( id , szMessage[] , iLen , bool:bList ){
	if( bList ){
		formatex( szMessage , iLen , "Dodatkowe obrazenia, gdy trafisz kogos od tylu")
	}
	else{
		formatex( szMessage , iLen , "Dodatkowe %i obrazen, gdy trafisz kogos od tylu", floatround(15+diablo_get_user_str(id)*2*iDamage[id]/10.0) )
	}
}
